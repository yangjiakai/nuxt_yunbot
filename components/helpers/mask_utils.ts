...
function toImageData(input: any, width: number, height: number, mode?: string) {
  let r, g, b, a;
...
export function rleToImage(
  ...
  return imageDataToImage(toImageData(input, width, height, mode));
}
...
export function rleToCanvas(
  ...
  return imageDataToCanvas(toImageData(input, width, height, mode));
}
