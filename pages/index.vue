...
<template>
  <div class="pa-5">
    ...
    <v-card variant="outlined" class="mb-5">
      <v-card-title class="px-5 py-8 font-weight-bold"
        >AI 图像生成</v-card-title
      >
      <v-card-text>
        <v-row>
          <v-col
            align="center"
            v-for="item in generateTools"
            cols="12"
            sm="6"
            md="4"
            lg="3"
            ...
            :key="item.link"
          >
            <WaveCard @click="router.push('/ai-image/' + item.link)">
              <template #card-body>
                <div
                  class="h-full d-flex flex-column align-center justify-space-around"
                >
                  <Icon size="80" name="solar:figma-bold-duotone" />
                  <h1 class="text-h4 font-weight-bold">{{ item.text }}</h1>

                  <h5 class="text-h6">{{ item.link }}</h5>
                </div>
              </template>
            </WaveCard>
          </v-col>
    ...
    <!-- 图像编辑 -->
    <v-card variant="outlined" class="mb-5">
      <v-card-title class="px-5 py-8 font-weight-bold">图像编辑</v-card-title>
      <v-card-text>
        <v-row>
          <v-col
            align="center"
            ...
            :key="item.link"
          >
            <WaveCard @click="router.push('/image-edit/' + item.link)">
              <template #card-body>
                <div
                  class="h-full d-flex flex-column align-center justify-space-around"
                >
    ...
    <!-- AI 设计 -->
    <v-card variant="outlined" class="mb-5">
      <v-card-title class="px-5 py-8 font-weight-bold">AI 设计</v-card-title>
      <v-card-text>
        <v-row>
          <v-col
            align="center"
